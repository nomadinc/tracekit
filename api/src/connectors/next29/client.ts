...
export class Next29Client {
  ...
  constructor(config: Next29ClientConfig, dependencies: Next29ClientDependencies = {}) {
    ...
    if (custom && !config.allowCustomBaseUrl) {
      throw next29ConfigurationError("A custom 29Next base URL requires explicit allowCustomBaseUrl configuration.");
    }
  ...
  async listOrders(
    options: { cursor?: string | null; query?: Record<string, string | number | boolean | null | undefined> } = {},
    context: Next29RequestContext = {},
  ): Promise<Next29Page<Next29OrderSummary>> {
    const params = new URLSearchParams();
    if (options.cursor) params.set("cursor", options.cursor);
    for (const [key, value] of Object.entries(options.query ?? {})) {
      if (value !== null && value !== undefined && value !== "") params.set(key, String(value));
    }
    const response = await this.request("orders", "orders/", params, context);
    return parsePage<Next29OrderSummary>(response);
  }

  async getOrder(number: string, context: Next29RequestContext = {}): Promise<{
    item: Next29Order;
    providerRequestId: string | null;
    correlationId: string;
  }> {
  ...
  async *iterateOrders(
    options: { maxPages?: number; query?: Record<string, string | number | boolean | null | undefined> } = {},
    context: Next29RequestContext = {},
  ) {
    ...
    for (let pageNumber = 0; pageNumber < maxPages; pageNumber += 1) {
      const page = nextUrl
        ? parsePage<Next29OrderSummary>(await this.requestAbsolute("orders", nextUrl, context))
        : await this.listOrders({ query: options.query }, context);
      yield page;
      if (!page.next) return;
  ...
  private validatePaginationUrl(value: string) {
    ...
    try {
      ...
      throw next29ConfigurationError("29Next pagination URL was invalid.");
    }
  ...
  private async requestUrl(resource: string, url: URL, context: Next29RequestContext): Promise<TransportResult> {
    ...
    for (let attempt = 1; attempt <= this.maxAttempts; attempt += 1) {
      try {
        ...
        const error = next29ErrorForStatus({
          status: response.status,
          message: providerMessage(body) || `29Next ${resource} request failed with HTTP ${response.status}.`,
          resource,
          correlationId,
          providerRequestId,
          retryAfterMs,
        ...
        if (attempt === this.maxAttempts) {
          throw next29ErrorForStatus({
            status: 503,
            message: `29Next ${resource} request failed: ${redactNext29Text(error instanceof Error ? error.message : error)}`,
            resource,
            correlationId,
          });
        }
    ...
    try {
      return await this.fetcher(url, {
        ...
        headers: {
          ...
          "X-29Next-Api-Version": this.apiVersion,
...
function parsePage<T>(response: TransportResult): Next29Page<T> {
  if (!isObject(response.body) || !Array.isArray(response.body.results)) {
    throw next29ConfigurationError("29Next list response did not contain a results array.");
  }
  const next = optionalUrl(response.body.next);
  const previous = optionalUrl(response.body.previous);
  return {
    results: response.body.results as T[],
    next,
    previous,
...
function parseJson(text: string, status: number): unknown {
  ...
  try {
    ...
    throw next29ConfigurationError(`29Next returned invalid JSON for HTTP ${status}.`);
  }
}
...
function providerMessage(body: unknown) {
  ...
  return typeof candidate === "string" ? redactNext29Text(candidate) : null;
}

function requestIdFrom(headers: Headers) {
  return headers.get("x-request-id") || headers.get("x-29next-request-id") || headers.get("request-id");
}

function parseRateLimit(headers: Headers) {
  return {
    limit: optionalInteger(headers.get("x-ratelimit-limit")),
    remaining: optionalInteger(headers.get("x-ratelimit-remaining")),
    retryAfterMs: parseRetryAfterMs(headers.get("Retry-After"), Date.now()),
  };
}
...
function retryDelay(attempt: number, retryAfterMs: number | null, random: () => number) {
...
function normalizeStore(value: unknown) {
  ...
  if (!/^[a-z0-9][a-z0-9-]{0,62}$/.test(store)) throw next29ConfigurationError("29Next store slug is invalid.");
  return store;
}
...
function safeUrl(value: string) {
  try {
    ...
    throw next29ConfigurationError("29Next base URL is invalid.");
  }
}

function positiveInteger(value: unknown, fallback: number, label: string) {
  const candidate = value === undefined ? fallback : Number(value);
  if (!Number.isInteger(candidate) || candidate < 1) throw next29ConfigurationError(`29Next ${label} must be a positive integer.`);
  return candidate;
}

function requiredId(value: unknown, label: string) {
  const result = String(value ?? "").trim();
  if (!result || result.length > 200) throw next29ConfigurationError(`29Next ${label} is invalid.`);
  return result;
}
