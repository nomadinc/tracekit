...
function requireIsoTimestamp(value: string) {
  ...
  if (!raw || !Number.isFinite(parsed)) {
    throw new Error("Shopify incremental checkpoint updatedAt must be an ISO-8601 timestamp.");
  }
...
function deriveCheckpoint<TNode extends { updatedAt: string }>(args: {
  resource: ShopifyIncrementalResource;
  previousUpdatedAt: string;
  nodes: TNode[];
  hasNextPage: boolean;
  endCursor: string | null;
}): ShopifyIncrementalCheckpoint {
  const newest = args.nodes.reduce((max, node) => {
  ...
  }, requireIsoTimestamp(args.previousUpdatedAt));
...
async function listIncrementalPage<TNode extends { updatedAt: string }>(args: {
  ...
  if (args.checkpoint.resource !== args.resource) {
    throw new Error(`Shopify checkpoint resource mismatch: expected ${args.resource}.`);
  }
  ...
  return {
    ...
    checkpoint: deriveCheckpoint({
      resource: args.resource,
      previousUpdatedAt: args.checkpoint.updatedAt,
      nodes,
      hasNextPage,
      endCursor,
    }),
...
export function initialShopifyCheckpoint(resource: ShopifyIncrementalResource, updatedAt: string): ShopifyIncrementalCheckpoint {
  return { resource, updatedAt: requireIsoTimestamp(updatedAt), cursor: null };
}

export function listShopifyProductsPage(
  client: Pick<ShopifyAdminClient, "graphql">,
  checkpoint: ShopifyIncrementalCheckpoint,
  pageSize?: number,
) {
...
export function listShopifyCustomersPage(
  client: Pick<ShopifyAdminClient, "graphql">,
  checkpoint: ShopifyIncrementalCheckpoint,
  pageSize?: number,
) {
...
export function listShopifyOrdersPage(
  client: Pick<ShopifyAdminClient, "graphql">,
  checkpoint: ShopifyIncrementalCheckpoint,
  pageSize?: number,
) {
