...
test("financial import monitor route uses an authenticated server-side proxy", () => {
  ...
  assert.match(navigation, /href: "\/dashboard\/financial-import-monitor", label: "Financial Imports"/);
  assert.match(page, /<FinancialImportMonitorClient \/>/);
  assert.match(page, /Loading Financial Imports/);
  assert.match(client, /Are your financial imports healthy\?/);
  assert.match(client, /Monitor whether financial connectors are importing successfully|Financial Imports tracks connector availability/);
  ...
  assert.match(client, /ImportHealthHero/);
  assert.match(client, /What needs your attention\?/);
  assert.match(client, /onClick=\{\(\) => onSelect\(card\)\}/);
  assert.match(client, /focusInventory\(card\.account_keys\[0\] \|\| null\)/);
  assert.match(client, /Recent import activity/);
  assert.match(client, /Connector inventory/);
  assert.match(client, /Show filters/);
  assert.match(client, /aria-expanded=\{filtersOpen\}/);
  assert.match(client, /aria-controls="financial-import-filters"/);
  assert.match(client, /AccountTable/);
  assert.match(client, /DetailPanel/);
  assert.match(client, /Imported financial events/);
  assert.match(client, /Technical details/);
  assert.match(client, /No financial connectors are configured/);
  assert.match(client, /No connector accounts match these filters/);
  assert.match(client, /No recent financial import activity/);
  assert.match(client, /Needs attention/);
  assert.doesNotMatch(client, /<h1[\s\S]*Financial Import Monitor/);
  assert.doesNotMatch(client, /SummaryCards/);
  assert.doesNotMatch(client, /Financial Event Breakdown/);
  assert.match(lib, /financialImportMonitorQuery/);
  assert.match(lib, /deriveFinancialImportHealth/);
  assert.match(lib, /buildFinancialImportIssueCards/);
  assert.match(lib, /recentFinancialImportActivity/);
...
test("financial reconciliation center routes, UI, and migration are wired safely", () => {
  ...
  assert.match(navigation, /href: "\/dashboard\/financial-reconciliation", label: "Financial Health"/);
  assert.match(page, /<FinancialReconciliationClient \/>/);
  assert.match(client, /Financial Health/);
  assert.match(client, /Can you trust this financial data\?/);
  assert.match(client, /What needs your attention\?/);
  assert.match(client, /Financial review queue/);
  assert.match(client, /Prioritized issues that need operator review or confirmation/);
  assert.match(client, /Financial impact/);
  assert.match(client, /Recent financial activity/);
  assert.match(client, /Financial ledger/);
  assert.match(client, /Search and inspect every refund, chargeback, fee, and reversal event/);
  assert.match(client, /Financial events/);
  assert.match(client, /Complete financial event history for the selected period/);
  assert.match(client, /Show advanced filters/);
  assert.match(client, /Hide advanced filters/);
  assert.match(client, /No financial issues need review/);
  assert.match(client, /All actionable items in this view have been reconciled or cleared/);
  assert.match(client, /No financial events found for this period/);
  assert.match(client, /Adjust the date range or filters to inspect other events/);
  assert.match(client, /Review context/);
  assert.match(client, /Ledger event details and reconciliation evidence/);
  assert.match(client, /document\.getElementById\("work-queue"\)/);
  assert.match(client, /document\.getElementById\("financial-ledger"\)/);
  assert.match(client, /setExplorerOpen\(true\)/);
  assert.doesNotMatch(client, /Advanced explorer/);
  assert.doesNotMatch(client, /href="#advanced-explorer"/);
  assert.match(client, /Automatically matched/);
  assert.match(client, /Confirm automatic match/);
  assert.match(client, /Order \/ reference/);
  assert.match(client, /Missing affiliate\/source/);
  assert.match(client, /Not reported/);
  assert.match(client, /Unknown account/);
  assert.match(client, /Multiple currencies/);
  assert.match(client, /Diagnostics do not change/);
  ...
  assert.match(lib, /deriveFinancialHealth/);
  assert.match(lib, /buildFinancialIssueCards/);
  assert.match(lib, /buildFinancialWorkQueue/);
  assert.match(lib, /financialEventDisplayLabel/);
  assert.match(lib, /Order #/);
  assert.match(lib, /processor reference/);
  assert.match(lib, /source event/);
  assert.match(lib, /netFinancialImpact/);
  assert.match(lib, /recentFinancialActivity/);
  assert.match(lib, /Partial view/);
  assert.match(lib, /No financial events/);
  assert.match(lib, /Review needed/);
  assert.match(lib, /Critical review/);
