  ...
  return {
    ...
    async preReservedRunPermitted(message) {
      ...
      if (message.evidence_only_recovery === true) {
        ...
        try { valid = message.operator_one_shot
        ? String((run as any).sync_type || "") === message.resource
          ...
          && Number(((run as any).metadata || {}).max_pages) === (message.ordering_verification ? 3 : normalAcceptance ? 5 : 8)
      ...
      if (!valid) { if (!orderingVerification) console.log("[TraceKit] commerce.queue.pre_reserved_rejected", { reserved_run_id: message.reserved_run_id, ...preReservedRunMatchDetails(run, message) }); console.log("[TraceKit] commerce pre-reserved validation", { event: "commerce.pre_reserved.validation_failed", run_id: message.reserved_run_id }); }
    ...
    async manualPermitted(message) {
      ...
      const [{ data: connection, error: connectionError }, { data: accounts, error: accountError }, { data: schedules, error: scheduleError }, { data: pauses, error: pauseError }, { count: activeRuns, error: activeRunError }, { count: liveActivation, error: liveActivationError }, { data: latest, error: latestError }] = await Promise.all([
      ...
      const schedule = schedules?.[0] as Record<string, unknown> | undefined;
      ...
      return String(connection?.provider || "") === "commas" && String(connection?.status || "") === "connected" && (accounts || []).length === 1 && String(accounts?.[0]?.id || "") === message.provider_account_id && String(schedule?.sync_frequency || "") === "manual" && schedule?.enabled === true && schedule?.activation_state !== "paused" && !Boolean(pauses?.[0]?.paused) && Number(activeRuns || 0) === 0 && Number(liveActivation || 0) === 0 && Number.isFinite(quotaRemaining) && quotaRemaining - 8 >= Number(schedule?.quota_minimum_remaining || 1000);
    },
    async operatorOneShotPermitted(message) {
      ...
      const [{ data: connection, error: connectionError }, { data: accounts, error: accountError }, { data: credential, error: credentialError }, { data: schedule, error: scheduleError }, { data: pause, error: pauseError }, { data: activeRuns, error: activeError }, { count: liveActivation, error: activationError }, { data: latest, error: latestError }, { count: schedulerControl, error: controlError }] = await Promise.all([
        ...
        orderingGateRead("credential", () => db.from("commerce_provider_credentials").select("id").eq("organization_id", message.organization_id).eq("connection_id", message.connection_id).is("revoked_at", null).limit(1)),
        orderingGateRead("schedule", () => db.from("commerce_sync_schedules").select("sync_frequency,enabled,activation_state,quota_minimum_remaining").eq("organization_id", message.organization_id).eq("connection_id", message.connection_id).eq("provider_account_id", message.provider_account_id).eq("resource", message.resource).limit(1).maybeSingle()),
      ...
      const requestBudget = orderingVerification ? 3 : message.normal_acceptance === true ? 5 : 8;
      const checks = {
...
async function router(req: Request, env: Env): Promise<Response> {
  ...
  if (path === "/internal/commerce/normal-continuous-acceptance" && req.method === "POST") {
    ...
    try {
      ...
      const message: CommerceQueueMessage = { schema_version:1, job_type:"commerce_continuous", provider:"commas", account_id:String(row.account_id), organization_id:String(row.organization_id), connection_id:"ea1c2313-6120-4692-84c5-ec3562e7dcf6", provider_account_id:String(row.provider_account_id), resource:"transactions", requested_mode:"continuous", scheduler_identity:String(row.scheduler_identity), requested_at:new Date().toISOString(), operator_one_shot:true, normal_acceptance:true, acceptance_cycle:true, max_pages:5, per_page:100, request_key:String(row.request_key), reserved_run_id:String(row.run_id) };
      ...
      return json({ ok:true, status:"queued", run_id:String(row.run_id), dispatch_source:"operator_one_shot", normal_acceptance:true, max_pages:5, per_page:100 },202);
    } catch {
  ...
  if (path === "/internal/commerce/one-shot-shadow" && req.method === "POST") {
    ...
    try {
      const body = await readJsonBody(req);
      if (body.confirmation !== "one-shot-continuous-shadow" || body.resource !== "transactions" || body.mode !== "continuous" || body.max_pages !== 8 || body.per_page !== 100) return json({ ok: false, error: "explicit_one_shot_confirmation_required" }, 400);
      const connectionId = String(body.connection_id || "");
      if (connectionId !== "ea1c2313-6120-4692-84c5-ec3562e7dcf6") return json({ ok: false, error: "connection_not_authorized" }, 403);
      const requestKey = String(body.request_key || "");
      if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(requestKey)) return json({ ok: false, error: "request_key_required" }, 400);
      if (!env.continuous_commerce) return json({ ok: false, error: "continuous_queue_unavailable" }, 503);
      if (env.TRACEKIT_COMMERCE_KILL_SWITCH !== "enabled") return json({ ok: false, error: "commerce_kill_switch_blocked" }, 409);
      const db = getSupabase(env);
      const { data: connection, error: connectionError } = await db.from("commerce_provider_connections").select("id,organization_id,account_id,provider,status").eq("id", connectionId).maybeSingle();
      if (connectionError || !connection) return json({ ok: false, error: "connection_unavailable" }, 409);
      const { data: accounts, error: accountError } = await db.from("commerce_provider_accounts").select("id").eq("organization_id", connection.organization_id).eq("connection_id", connectionId).eq("status", "active");
  ...
  if (path === "/internal/commerce/ordering-verification" && req.method === "POST") {
    ...
    try {
      const body = await readJsonBody(req);
      if (body.confirmation !== "verify-commas-provider-ordering") return json({ ok:false, error:"explicit_ordering_verification_confirmation_required" },400);
      if (env.TRACEKIT_COMMERCE_SCHEDULER_ENABLED !== "false" || env.TRACEKIT_COMMERCE_KILL_SWITCH !== "enabled") return json({ ok:false, error:"ordering_verification_controls_blocked" },409);
      if (!env.continuous_commerce) return json({ ok:false, error:"continuous_queue_unavailable" },503);
  ...
  if (path === "/v1/commerce/sync-now" && req.method === "POST") {
    ...
    try {
      const body = await readJsonBody(req);
      const ids = { account_id: String(body.account_id || ""), organization_id: String(body.organization_id || ""), connection_id: String(body.connection_id || ""), provider_account_id: String(body.provider_account_id || "") };
      if (![ids.account_id, ids.organization_id, ids.connection_id, ids.provider_account_id].every((value) => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value))) return json({ ok: false, error: "invalid_scope" }, 400);
      if (!env.continuous_commerce) return json({ ok: false, error: "continuous_queue_unavailable" }, 503);
      const message: CommerceQueueMessage = { schema_version: 1, job_type: "commerce_continuous", provider: "commas", ...ids, resource: "transactions", requested_mode: "continuous", scheduler_identity: `manual:${crypto.randomUUID()}`, requested_at: new Date().toISOString(), manual: true };
      const repository = getContinuousCommerceAdapterRepository(env);
      if (!await repository.manualPermitted?.(message)) return json({ ok: false, error: "manual_sync_not_permitted" }, 409);
      if (await repository.reserve(message) === "duplicate") return json({ ok: true, status: "duplicate" }, 200);
      await env.continuous_commerce.send(message);
      return json({ ok: true, status: "queued" }, 202);
    } catch (error: any) {
      return json({ ok: false, error: "manual_sync_failed", message: String(error?.message || error) }, 500);
    }
  }
