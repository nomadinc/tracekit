...
function record(resource: "orders" | "products" | "customers", payload: Record<string, unknown>): ShopifyPersistedRecord {
  return { ...scope, resource, providerObjectId: String(payload.id), providerUpdatedAt: "2026-08-30T20:00:00.000Z", payload: payload as any };
}
function provenance(resource: "orders" | "products" | "customers") {
  return { syncRunId: "run-1", page: { resource, nodes: [], checkpoint: { cursor: null, updatedAt: null, page: 1 }, hasNextPage: false } as ShopifySyncPage };
}
...
test("Shopify order normalization qualifies legacy platform identity by store and preserves provider GID", () => {
  const draft = normalizeShopifyOrderRecord(record("orders", {
    id: "gid://shopify/Order/123", name: "#1001", createdAt: "2026-08-30T19:00:00Z", displayFinancialStatus: "PAID",
    ...
    currentTotalTaxSet: { shopMoney: { amount: "0", currencyCode: "USD" } }, customer: { email: "buyer@example.com" },
...
test("Shopify product and customer normalization preserve durable provider identities", () => {
  const product = normalizeShopifyProductRecord(record("products", { id: "gid://shopify/Product/10", title: "Widget", variants: { nodes: [{ id: "gid://shopify/ProductVariant/11", title: "Blue", sku: "BLUE-1", price: "19.95" }] } }));
  ...
  const customer = normalizeShopifyCustomerRecord(record("customers", { id: "gid://shopify/Customer/20", firstName: "Ada", lastName: "Lovelace", email: "ada@example.com" }));
  assert.equal(customer.provider_customer_id, "gid://shopify/Customer/20");
  assert.equal(customer.display_name, "Ada Lovelace");
});

test("normalized writer persists evidence, canonical order mapping, lines, refund, and refund ledger", async () => {
  ...
  await writer([record("orders", {
    id: "gid://shopify/Order/123", name: "#1001", createdAt: "2026-08-30T19:00:00Z", displayFinancialStatus: "PAID",
    currentTotalPriceSet: { shopMoney: { amount: "99.50", currencyCode: "USD" } },
    lineItems: { nodes: [{ id: "gid://shopify/LineItem/1", quantity: 2, title: "Widget", sku: "W-1", product: { id: "gid://shopify/Product/10" }, variant: { id: "gid://shopify/ProductVariant/11" }, discountedTotalSet: { shopMoney: { amount: "40.00", currencyCode: "USD" } } }] },
    refunds: { nodes: [{ id: "gid://shopify/Refund/7", createdAt: "2026-08-30T20:30:00Z", totalRefundedSet: { shopMoney: { amount: "20.00", currencyCode: "USD" } }, transactions: { nodes: [{ id: "gid://shopify/OrderTransaction/8", amountSet: { shopMoney: { amount: "20.00", currencyCode: "USD" } } }] } }] },
  })], provenance("orders"));
  ...
  assert.equal(JSON.parse(String(evidenceWrite?.init?.body)).sync_run_id, "run-1");

  const mappingWrites = calls.filter((call) => call.url.endsWith("/rest/v1/commerce_source_mappings") && call.init?.method === "POST");
  assert.equal(mappingWrites.length, 2);
  assert.deepEqual(mappingWrites.map((call) => JSON.parse(String(call.init?.body)).canonical_object_type).sort(), ["order", "refund"]);

  const orderWrite = calls.find((call) => call.url.includes("platform_orders?on_conflict=platform_order_id"));
  const orderBody = JSON.parse(String(orderWrite?.init?.body));
  assert.equal(orderBody.organization_id, scope.organizationId);
  assert.equal(orderBody.platform_order_id, "shopify:store-one.myshopify.com:gid://shopify/Order/123");
  assert.ok(orderBody.canonical_order_id);
  assert.ok(orderBody.source_mapping_id);
  assert.ok(orderBody.evidence_id);

  const lineWrite = calls.find((call) => call.url.includes("commerce_order_lines?on_conflict="));
  const lines = JSON.parse(String(lineWrite?.init?.body));
  assert.equal(lines[0].source_line_key, "gid://shopify/LineItem/1");
  assert.equal(lines[0].quantity, 2);
  assert.equal(lines[0].gross_amount, 40);
  assert.equal(lines[0].unit_amount, 20);
  assert.equal(lines[0].organization_id, scope.organizationId);

  const refundWrite = calls.find((call) => call.url.includes("commerce_refund_events?on_conflict="));
  const refund = JSON.parse(String(refundWrite?.init?.body));
  assert.equal(refund.provider_refund_id, "gid://shopify/Refund/7");
  assert.equal(refund.amount, 20);
  assert.equal(refund.provider_payment_id, "gid://shopify/OrderTransaction/8");

  const conversionWrite = calls.find((call) => call.url.endsWith("/rest/v1/conversions") && call.init?.method === "POST");
  const conversion = JSON.parse(String(conversionWrite?.init?.body));
  assert.equal(conversion.ledger_type, "refund");
  assert.equal(conversion.amount, -20);
  assert.equal(conversion.connector_id, "shopify");
  assert.equal(conversion.idempotency_key, "refund:gid://shopify/Refund/7");
});
...
test("customer writer updates an existing provider identity instead of relying on partial-index upsert", async () => {
  ...
  assert.ok(JSON.parse(String(identityWrite?.init?.body)).evidence_id);
