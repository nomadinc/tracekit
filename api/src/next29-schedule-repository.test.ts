...
test("29Next schedule adapter maps runtime resources to provider schedule identities and preserves checkpoint state", async () => {
  ...
  const control=createNext29IncrementalControl({
    ...
    async finishSchedule(){},async failSchedule(){}
  });
...
test("29Next schedule adapter maps completion and failure without inventing checkpoint advancement", async () => {
  let finish:any=null; let fail:any=null;
  const control=createNext29IncrementalControl({async claimSchedule(){return null},async finishSchedule(input){finish=input},async failSchedule(input){fail=input}});
