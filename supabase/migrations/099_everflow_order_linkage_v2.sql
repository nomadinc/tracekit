...
create or replace function public.run_everflow_order_reconciliation_batch_v2(
...
as $$
declare
  v_algorithm constant text := 'deterministic_order_v2';
...
begin
  ...
  if not pg_try_advisory_xact_lock(hashtext('everflow_order_backfill_v2'), hashtext(p_connection_id::text)) then
  ...
  with batch as materialized (
    ...
    select *,
      case
        ...
        when direct_candidate_count=0 and strict_candidate_count=0 and tight_candidate_count=1 then tight_order_id
        else null
      ...
      case
        ...
        when strict_candidate_count=1 or tight_candidate_count=1 then 'medium_confidence'
        when strict_candidate_count>1 or tight_candidate_count>1 then 'needs_review'
        else 'unmatched'
      end confidence_band,
      case
    ...
    select gen_random_uuid(),organization_id,connection_id,event_id,v_algorithm,confidence_band,candidate_count,
      ...
      jsonb_build_object(
        ...
        'confidence',case match_method when 'existing_mapping' then 1.0 when 'transaction_id' then 1.0 when 'email_time_amount' then 0.85 when 'email_time_30m' then 0.80 else 0.0 end,
        'event_class',case when is_commerce then 'commerce_value' else 'upper_funnel' end,
        'tight_email_window_minutes',30,
  ...
  select e.organization_id,e.connection_id,e.provider_account_id,'everflow_conversion',e.source_identity,
    ...
    'everflow-order-linkage-v2','active',jsonb_build_object('algorithm_version',v_algorithm,'match_method',r.evidence_factors->>'match_method')
  ...
  select e.organization_id,e.connection_id,e.provider_account_id,'everflow_transaction',btrim(e.transaction_id),
    'order',r.matched_canonical_order_id,e.conversion_at,v_now,e.conversion_at,e.payload_hash,
    'everflow-order-linkage-v2','active',jsonb_build_object('algorithm_version',v_algorithm)
  from public.everflow_order_reconciliations r
  join public.everflow_conversion_events e on e.id=r.event_id and e.organization_id=r.organization_id
  where r.connection_id=p_connection_id and r.algorithm_version=v_algorithm and r.reconciled_at=v_now
    and r.matched_canonical_order_id is not null
...
revoke all on function public.run_everflow_order_reconciliation_batch_v2(uuid,integer) from public;
grant execute on function public.run_everflow_order_reconciliation_batch_v2(uuid,integer) to service_role;
