-- Correct migration 066 root validation for roots that self-reference.
...
begin
  ...
  if nullif(v_root.metadata->>'quota_bootstrap_original_run_id', '') is not null
     and v_root.metadata->>'quota_bootstrap_original_run_id' <> p_root_run_id::text then
