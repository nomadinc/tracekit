-- Restore the 29Next scheduler RPC control plane when migration history/schema
-- drift leaves commerce_sync_schedules columns present but the checked-in RPCs absent.
-- Source of truth: the original migrations already tracked in main.
-- This migration does not create an external dispatcher and does not enable schedules.

-- BEGIN next29_incremental_scheduler_foundation
...
-- END next29_incremental_scheduler_foundation

-- BEGIN next29_scheduler_dispatch_runtime
...
-- END next29_scheduler_dispatch_runtime
