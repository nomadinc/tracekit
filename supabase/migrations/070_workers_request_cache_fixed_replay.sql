-- One final post-fix replay authorization for the cancelled Worker request
-- diagnostic run. This is isolated from ordinary quota-bootstrap retries.
create or replace function public.replay_commerce_workers_request_cache_fixed()
returns table (
...
declare
  ...
  v_scheduler_identity text := 'workers-request-cache-fixed:' || gen_random_uuid()::text;
begin
  perform pg_advisory_xact_lock(hashtextextended('commerce-workers-request-cache-fixed:37e77c92-f333-437a-91a9-2acb38e73eca', 0));

  select * into v_source
    from public.commerce_sync_runs r
   where r.id = '37e77c92-f333-437a-91a9-2acb38e73eca'::uuid
   for update;
  if not found then raise exception 'post-fix source run not found' using errcode = 'P0002'; end if;
  if v_source.status <> 'cancelled' or v_source.mode <> 'continuous' then
    raise exception 'post-fix source run is not cancelled continuous' using errcode = '55000';
  end if;
  if coalesce(v_source.metadata->>'quota_bootstrap_provider_requests', '') <> '0' then
    raise exception 'post-fix source has provider requests' using errcode = '55000';
  end if;
  if coalesce(v_source.metadata->>'workers_request_cache_fixed_replay_consumed', 'false') = 'true' then
    raise exception 'post-fix replay already consumed' using errcode = '23505';
  end if;

  select nullif(v_source.metadata->>'account_id', '')::uuid into v_account_id;
  if v_account_id is null then raise exception 'post-fix account scope missing' using errcode = '22023'; end if;
  ...
  if not found or lower(coalesce(v_connection.provider, '')) <> 'commas'
    ...
    raise exception 'post-fix connection scope invalid' using errcode = '55000';
  end if;
  ...
  ) then raise exception 'post-fix provider account scope invalid' using errcode = '55000'; end if;
  ...
  if v_controls <> 0 then raise exception 'post-fix scheduler control enabled' using errcode = '55000'; end if;
  ...
  if v_schedules <> 0 then raise exception 'post-fix schedule enabled' using errcode = '55000'; end if;
  ...
  if v_active_runs <> 0 then raise exception 'post-fix active run exists' using errcode = '55000'; end if;
  ...
  if v_live_activations <> 0 then raise exception 'post-fix live activation exists' using errcode = '55000'; end if;
  ...
  if nullif(v_latest_metadata->>'rate_limit_end', '') is not null then
    raise exception 'post-fix quota is known' using errcode = '55000';
  end if;

  update public.commerce_sync_runs r
     set metadata = coalesce(r.metadata, '{}'::jsonb) || jsonb_build_object(
       'workers_request_cache_fixed_replay_consumed', true,
       'workers_request_cache_fixed_replay_reason', 'workers_request_cache_fixed',
       'workers_request_cache_fixed_original_run_id', r.id::text
     ), updated_at = now()
  ...
  insert into public.commerce_sync_runs (
    ...
    jsonb_build_object(
      ...
      'workers_request_cache_fixed_replay', true,
      'workers_request_cache_fixed_replay_reason', 'workers_request_cache_fixed',
      'reason', 'workers_request_cache_fixed'
    )
  );
...
revoke all on function public.replay_commerce_workers_request_cache_fixed() from public, anon, authenticated;
grant execute on function public.replay_commerce_workers_request_cache_fixed() to service_role;
