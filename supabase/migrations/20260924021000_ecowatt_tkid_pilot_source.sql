...
begin
 ...
 select bc.account_id,bc.organization_id,bc.id,'tksrc_ecowatt_pilot_v1','production','shadow',array['https://buyecowatt.com'],1,'essential',120,'tkid-v1-review-required','1.0.0'
 from public.tracekit_business_contexts bc where bc.status='active' order by (bc.name ilike '%ecowatt%') desc,bc.created_at asc limit 1
 ...
 if v_source is null then raise exception 'No active business context available for EcoWatt pilot'; end if;
 ...
 select s.account_id,s.organization_id,s.business_context_id,s.id,'https://buyecowatt.com','frontend','pending','dns_txt','unissued'
