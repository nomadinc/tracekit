...
export function ShellDrawerProvider({ children }: { children: React.ReactNode }) {
  ...
  const previousRef = React.useRef({ pathname, identityId: session.identity.id, organizationId: session.activeOrganizationId, businessContextId: session.activeBusinessContextId });
  React.useEffect(() => {
    const next = { pathname, identityId: session.identity.id, organizationId: session.activeOrganizationId, businessContextId: session.activeBusinessContextId };
    const previous = previousRef.current;
    if (drawer && (previous.pathname !== next.pathname || previous.identityId !== next.identityId || previous.organizationId !== next.organizationId || previous.businessContextId !== next.businessContextId)) setDrawer(null);
    previousRef.current = next;
  }, [drawer, pathname, session.activeBusinessContextId, session.activeOrganizationId, session.identity.id]);
  React.useEffect(() => {
    if (!drawer) return;
    function onKeyDown(event: KeyboardEvent) { if (event.key === "Escape") setDrawer(null); }
