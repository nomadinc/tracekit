...
const pill: Record<string, string> = {
  ...
  missing: "border-rose-400/20 bg-rose-400/10 text-rose-200",
};
                  ...
                  {first ? (
                    <>
                      ...
                      <div className="mt-auto flex flex-wrap items-center gap-4 pt-5">
                        ...
                        {provider.provider === "next29" ? <button type="button" onClick={() => setConnectProvider("next29")} className="text-xs font-semibold text-slate-400 hover:text-slate-200">Add Store</button> : null}
                      </div>
                    </>
                  ) : (
                    <>
  ...
  return (
    <Dialog title={`Connect ${providerName}`} close={close}>
      <form onSubmit={submit} className="space-y-4">
        <Field label="Display name"><input required name="displayName" autoComplete="off" className="input" placeholder={isEverflow ? "Accufy Everflow" : isShopify ? "Main Shopify Store" : isNext29 ? "StemLabs v3" : "Push Button Systems"} /></Field>
        ...
        {isNext29 ? <Field label="Store"><input required name="store" autoComplete="off" className="input" placeholder="stemlabs or stemlabs.29next.store" /></Field> : null}
        {isNext29 ? <Field label="29Next Admin API access token"><input required minLength={8} name="accessToken" type="password" autoComplete="new-password" className="input" placeholder="Stored encrypted; never shown again" /></Field> : null}
