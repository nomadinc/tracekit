...
const attention: MissionItem[] = [
  { id: "shipping", business: "Bullseye", businessContextId: "offer-bullseye", mark: "B", title: "Shipping Margin declined 18%", detail: "$12,480 below the prior period", tone: "Warning", action: "Open Offer", destination: "offer", offerDeepLink: { focus: "profit-drivers", driverId: "actual-shipping-cost", drawerId: "driver:actual-shipping-cost" }, question: "Should I investigate this shipping change?", explanation: "Actual Shipping Cost increased while Shipping Charged remained unchanged.", evidence: ["3,240 matched Orders", "Carrier cost imports", "Shipping Charged", "Packaging costs"], intelligence: { comparison: "Shipping Margin is $3.40 per Order below the 30-day average.", recommendation: "Review shipping terms before increasing Meta spend.", confidence: "Strong evidence" } },
  { id: "meta", business: "Meta", businessContextId: "offer-bullseye", mark: "M", title: "CPA increased 22%", detail: "Three consecutive days above target", tone: "Warning", action: "Investigate", destination: "offer", offerDeepLink: { focus: "traffic-sources", trafficSourceId: "meta-facebook", drawerId: "traffic:meta-facebook" }, question: "Should I investigate this acquisition change?", explanation: "Meta acquisition cost increased after the latest Campaign budget change.", evidence: ["Attributed Meta Spend", "Matched Customer acquisitions", "Campaign change record"], intelligence: { comparison: "CPA is $11.20 above the previous seven-day period.", recommendation: "Inspect the affected Campaigns before adding budget.", confidence: "Moderate evidence" } },
  { id: "valuerx", business: "ValueRx", businessContextId: "offer-valuerx-individual", mark: "VR", title: "Highest weekly Profit", detail: "$112,840 Reconciled Profit", tone: "Opportunity", action: "View", destination: "offer", offerDeepLink: { focus: "traffic-sources", trafficSourceId: "affiliates", drawerId: "traffic:affiliates" }, question: "Should I invest more in ValueRx?", explanation: "ValueRx currently combines the strongest Profit Margin with lower CPA and refunds.", evidence: ["Reconciled Profit", "Traffic Source cohorts", "Refund Financial Events"], intelligence: { comparison: "Profit Margin is 9.3 points above Bullseye.", recommendation: "Review ValueRx Affiliates for the next bounded budget test.", confidence: "Strong evidence" } },
...
const snapshot: MissionControlSnapshot = {
  ...
  recentActivity: [
    ...
    { id: "a3", type: "Order", title: "Order TK-10482", detail: "Processor fee Evidence", destination: "order", businessContextId:"offer-bullseye", orderDeepLink:{orderId:"ord-123",focus:"ledger",lineId:"processor-fees",drawerId:"financial:processor-fees"} },
  ...
  recentSearches: [
    ...
    { id: "s4", type: "Order", value: "TK-10482", destination: "order", businessContextId:"offer-bullseye", orderDeepLink:{orderId:"ord-123"} },
  ],
};
