...
export async function persistEverflowEventReversalHistory(input: {
  ...
  for (let offset = 0; offset < history.length; offset += HISTORY_BATCH_SIZE) {
    ...
    await commercePersistenceRequest(
      ...
      { method: "POST", headers: { Prefer: "resolution=ignore-duplicates,return=representation" }, body: JSON.stringify(batch) },
    );
  }
