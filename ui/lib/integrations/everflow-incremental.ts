...
export async function markEverflowIncrementalAttempt(input:{accountId:string;organizationId:string;connectionId:string;providerAccountId:string;attemptedAt:string;window:{from:string;targetTo:string;overlapDays:number;bootstrap:boolean}}) {
  const row={account_id:input.accountId,organization_id:input.organizationId,connection_id:input.connectionId,provider_account_id:input.providerAccountId,resource:RESOURCE,last_attempted_at:input.attemptedAt,normalizer_version:"everflow-conversion-v1",evidence_contract_version:"everflow-raw-v1",status:"unknown",attribution_source_state:"available",recent_source_ids:[],page_fingerprints:{},last_stability_boundary:{incrementalCursorFrom:input.window.from,incrementalTargetTo:input.window.targetTo,overlapDays:input.window.overlapDays,bootstrap:input.window.bootstrap},warnings:[],updated_at:input.attemptedAt};
...
export async function markEverflowIncrementalChunkSuccess(input:{organizationId:string;connectionId:string;providerAccountId:string;completedAt:string;syncRunId:string;from:string;to:string;targetTo:string;overlapDays:number;bootstrap:boolean;seen:number}) {
  ...
  if(finalDay){Object.assign(patch,{last_successful_at:input.completedAt,last_stability_boundary:{providerFrom:input.from,providerTo:input.targetTo,overlapDays:input.overlapDays,syncRunId:input.syncRunId},last_stopping_reason:"window_complete",status:"current"});}
  else{Object.assign(patch,{last_stability_boundary:{incrementalCursorFrom:nextDay(input.from),incrementalTargetTo:input.targetTo,overlapDays:input.overlapDays,bootstrap:input.bootstrap,lastChunkRunId:input.syncRunId},last_stopping_reason:"chunk_complete",status:"stale"});}
