...
async function runQueuedHistoricalImport(runtime:EverflowClickRuntime){
  ...
  try{
    ...
    await commercePersistenceRequest(`commerce_sync_runs?id=eq.${encodeURIComponent(String(candidate.id))}`,{method:"PATCH",body:JSON.stringify({status:"completed",completed_at:new Date().toISOString(),lease_owner:null,lease_expires_at:null,heartbeat_at:new Date().toISOString(),records_seen:seen,records_created:persisted,pages_completed:page,metadata:{...metadata,total_count:total,result:{seen,persisted,pages:page}}})});
    return{runId:candidate.id,status:"completed",seen,persisted,pages:page};
