...
export type SafeSyncRun = { id: string; connectionId: string; connectionName: string; provider: string; mode: string; resource: string; status: string; startedAt: string | null; completedAt: string | null; pagesCompleted: number; recordsSeen: number; recordsCreated: number; recordsUpdated: number; recordsUnchanged: number; recordsFailed: number; warnings: number; providerRequests: number; stoppingReason: string | null; freshnessResult: string | null; leaseActive: boolean; heartbeatAt: string | null; errorSummary: string | null; nonOrderEvents?: number | null; unmatchedCommerce?: number | null; matchedCommerce?: number | null; ambiguousCommerce?: number | null; conflictCommerce?: number | null };
...
export const NEXT29_CAPABILITIES: SafeCapability[] = [
  { name: "Orders", state: "supported", reason: "Bounded Admin API list, detail ingestion, and signed order.created webhook ingestion are implemented." },
