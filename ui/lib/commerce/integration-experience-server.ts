...
async function loadConnectionExperienceRow(organizationName: string, row: Row, canManage: boolean, canManageOrigins: boolean): Promise<ConnectionExperience> {
  ...
  const syncRuns = runs.map((run): SafeSyncRun => {
    const metadata = object(run.metadata); const everflow = object(metadata.everflow); const linkage = object(everflow.linkage); const isEverflow = provider === "everflow";
    return { id:String(run.id),connectionId:id,connectionName:String(row.display_name),provider,mode:String(run.mode),resource:String(run.sync_type),status:String(run.status),startedAt:text(run.started_at),completedAt:text(run.completed_at),pagesCompleted:number(run.pages_completed),recordsSeen:number(run.records_seen),recordsCreated:number(run.records_created),recordsUpdated:number(run.records_updated),recordsUnchanged:number(run.records_unchanged),recordsFailed:number(run.records_failed),warnings:number(run.warnings_count),providerRequests:number(run.provider_request_count),stoppingReason:text(run.stopping_reason),freshnessResult:text(run.freshness_result),leaseActive:Boolean(run.lease_owner&&run.lease_expires_at&&new Date(String(run.lease_expires_at))>new Date()),heartbeatAt:text(run.heartbeat_at),errorSummary:text(run.last_error_summary),nonOrderEvents:isEverflow?nullableNumber(linkage,"non_order"):null,unmatchedCommerce:isEverflow?nullableNumber(linkage,"unmatched"):null,matchedCommerce:isEverflow?nullableNumber(linkage,"matched"):null,ambiguousCommerce:isEverflow?nullableNumber(linkage,"ambiguous"):null,conflictCommerce:isEverflow?nullableNumber(linkage,"conflict"):null };
  });
  const readinessRecord=activation.find(item=>item.workspace==="orders"); const evidenceMap=(readinessRecord?.readiness_evidence||{}) as Record<string,{passed?:boolean;evidence?:string;at?:string}>;
  const latestContinuous=syncRuns.find(run=>run.mode==="continuous"&&run.status==="completed"); const deepProof=syncRuns.find(run=>run.mode==="deep_reconciliation"&&["completed","completed_with_warnings"].includes(run.status));
  ...
  const readiness:SafeReadinessGate[]=gates.map(([gateId,label])=>{const derived=gateId==="connection_verified"?String(row.status)==="connected":gateId==="credential_active"?Boolean(activeCredential):gateId==="tenant_scope"?accounts.length===1:gateId==="continuous_overlap"?Boolean(latestContinuous?.stoppingReason==="stable_known_boundary"):gateId==="deep_reconciliation"?Boolean(deepProof):gateId==="refund_normalization"?Boolean(latestContinuous&&latestContinuous.recordsFailed===0):gateId==="investigation_refresh"?Boolean(freshnessRows[0]):false;return{id:gateId,label,status:derived||evidenceMap[gateId]?.passed?"passed":gateId==="live_activation"?"not_required":"pending",explanation:evidenceMap[gateId]?.evidence||"Server-owned Connection scope is intact.",evidenceAt:evidenceMap[gateId]?.at||latestContinuous?.completedAt||null};});
  const latest=syncRuns[0]; const freshness=freshnessRows[0]; const activeSchedules=schedules.filter(schedule=>Boolean(schedule.enabled)&&String(schedule.activation_state)==="enabled"); const scheduleForSummary=activeSchedules[0]||schedules[0];
  const syncFrequency=(scheduleForSummary?String(scheduleForSummary.sync_frequency||"hourly"):"not_configured") as SyncFrequency;
  const lastEnqueuedAt=text(scheduleForSummary?.last_enqueued_at); const nextScheduleTimes=(activeSchedules.length?activeSchedules:schedules).map(schedule=>text(schedule.next_overlap_at)).filter((value):value is string=>Boolean(value)).map(value=>new Date(value)).filter(value=>!Number.isNaN(value.getTime())).sort((a,b)=>a.getTime()-b.getTime());
  const nextSyncAt=syncFrequency==="manual"||syncFrequency==="not_configured"?null:nextScheduleTimes[0]?.toISOString()||(lastEnqueuedAt?new Date(new Date(lastEnqueuedAt).getTime()+frequencyMinutes(syncFrequency)*60_000).toISOString():null);
  return { id,provider,displayName:String(row.display_name),environment:String(row.environment),status:String(row.status),organizationName,syncFrequency,nextSyncAt,providerAccountLabel:accounts[0]?String(accounts[0].provider_account_label||accounts[0].provider_account_external_id):null,lastVerifiedAt:text(row.last_success_at),lastSyncAt:latest?.completedAt||latest?.startedAt||null,capabilities:provider==="commas"?COMMAS_CAPABILITIES:[],syncRuns,credential:activeCredential?{status:"active",createdAt:text(activeCredential.created_at),rotatedAt:text(activeCredential.rotated_at),version:number(activeCredential.encryption_version)}:{status:credentials.length?"revoked":"missing",createdAt:null,rotatedAt:null,version:null},readiness,canManage,productionReadiness:{schedulerState:String(controls[0]?.activation_state||scheduleForSummary?.activation_state||"disabled") as ConnectionExperience["productionReadiness"]["schedulerState"],connectionPaused:Boolean(pauses[0]?.paused),quotaMinimumRemaining:scheduleForSummary?.quota_minimum_remaining==null?null:number(scheduleForSummary.quota_minimum_remaining),deepRequestBudget:scheduleForSummary?.deep_request_budget==null?null:number(scheduleForSummary.deep_request_budget),blockers:[...(!controls.length?["Production control not configured"]:[]),...(!schedules.length?["Schedule policy not configured"]:[]),...(Boolean(pauses[0]?.paused)?["Connection paused"]:[])]},tkidOrigins:{sourceId:tkidSource?String(tkidSource.id):null,sourceState:String(tkidSource?.status||"disabled"),origins:tkidOrigins,blockers:originBlockers,canManage:canManageOrigins},freshness:freshness?{status:String(freshness.status) as ConnectionExperience["freshness"]["status"],lastAttemptedAt:text(freshness.last_attempted_at),lastSuccessfulAt:text(freshness.last_successful_at),lastProviderObservationAt:text(freshness.last_provider_observation_at),lastNormalizedRecordAt:text(freshness.last_normalized_record_at),latestProviderTransactionAt:text(freshness.latest_provider_transaction_at),providerTotal:freshness.provider_total_observed==null?null:number(freshness.provider_total_observed),lastDeepReconciliationAt:text(freshness.last_deep_reconciliation_at),stoppingReason:text(freshness.last_stopping_reason),attributionSourceState:String(freshness.attribution_source_state) as ConnectionExperience["freshness"]["attributionSourceState"],deepReconciliationRequired:String(freshness.status)==="degraded"}:{status:"unknown",lastAttemptedAt:null,lastSuccessfulAt:null,lastProviderObservationAt:null,lastNormalizedRecordAt:null,latestProviderTransactionAt:null,providerTotal:null,lastDeepReconciliationAt:null,stoppingReason:null,attributionSourceState:"unavailable",deepReconciliationRequired:false},diagnostics:{latestRequestStatus:row.last_error_at?"failed":row.last_success_at?"succeeded":null,latencyMs:null,providerRequestIdPresent:false,retryCount:0,rateLimitRemaining:null,rateLimitReset:null,sanitizedError:text(row.last_error_code),activeRun:Boolean(latest&&["pending","running","paused"].includes(latest.status)),leaseOwnerPresent:Boolean(latest?.leaseActive),heartbeatAge:latest?.heartbeatAt||null,stalled:Boolean(latest?.status==="running"&&!latest.leaseActive),pendingCheckpoints:checkpoints.filter(item=>item.state==="pending").length,failedCheckpoints:checkpoints.filter(item=>item.state==="failed").length,evidenceReferences:evidence.filter(item=>!item.deleted_at).length,missingEvidenceReferences:evidence.filter(item=>!item.storage_reference).length,hashState:evidence.length?"pending":"unavailable"} };
}
export async function loadSyncRuns():Promise<SafeSyncRun[]>{return(await loadConnectionExperiences()).flatMap(connection=>connection.syncRuns);}
