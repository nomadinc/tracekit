...
export async function runContinuousCommasSync(options:{mode?:"continuous"|"deep_reconciliation";maxPages?:number;maxProviderRequests?:number;scheduledDeepSchedule?:{scheduleId:string;scheduleVersion:number};overlapPages?:number;perPage?:number;paceMs?:number;requestKey?:string;bootstrap?:boolean;evidenceOnlyRecovery?:boolean;expectedScope?:{organizationId:string;connectionId:string;providerAccountId:string}}={}):Promise<ContinuousSyncResult> {
  ...
  const scheduledDeepSchedule=options.scheduledDeepSchedule;
  if(mode==="deep_reconciliation"&&(!scheduledDeepSchedule||!uuidPattern.test(scheduledDeepSchedule.scheduleId)||!Number.isSafeInteger(scheduledDeepSchedule.scheduleVersion)||scheduledDeepSchedule.scheduleVersion<1))throw new Error("Scheduled deep reconciliation identity is invalid.");
  if(mode!=="deep_reconciliation"&&scheduledDeepSchedule)throw new Error("Scheduled deep reconciliation identity is invalid.");
