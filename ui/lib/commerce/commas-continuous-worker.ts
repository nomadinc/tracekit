...
export async function runContinuousCommasSync(options:{mode?:"continuous"|"deep_reconciliation";maxPages?:number;overlapPages?:number;perPage?:number;paceMs?:number;requestKey?:string;bootstrap?:boolean;expectedScope?:{organizationId:string;connectionId:string;providerAccountId:string}}={}):Promise<ContinuousSyncResult> {
  ...
  try {
    ...
    console.log("[TraceKit] commerce run transition",{event:"commerce.run.transition",result:(transitioned as unknown[])?.[0]===true?"succeeded":"not_applied",status});
    ...
    await db("rpc/transition_commerce_sync_run",{method:"POST",body:JSON.stringify({p_run_id:runId,p_organization_id:scope.organizationId,p_connection_id:scope.connectionId,p_lease_owner:owner,p_transition:"failed",p_error_code:"continuous_sync_failed",p_error_summary:"Continuous Commerce sync stopped safely."})}).then((transitioned)=>console.log("[TraceKit] commerce run transition",{event:"commerce.run.transition",result:(transitioned as unknown[])?.[0]===true?"succeeded":"not_applied",status:"failed"})).catch((error)=>console.log("[TraceKit] commerce run transition failed",{event:"commerce.run.transition_failed",errorCode:String((error as Error)?.message||"transition_error").replace(/[^a-zA-Z0-9_.-]/g,"_").slice(0,80)}));
