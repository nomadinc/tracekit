...
async function main(){
  ...
  const fingerprint=ordFrozenBaselineFingerprint({scope:{organizationId,connectionId,providerAccountId},window,evidence:selected.rows.map(row=>[row.observed_at,row.id,row.payload_hash]),records:records.map(row=>[row.public_transaction_id,row.transaction_id,row.payload_hash]).sort(),numericMappings:[...numericMappings].sort((a,b)=>`${a.source_object_id}:${a.canonical_object_id}:${a.state}`.localeCompare(`${b.source_object_id}:${b.canonical_object_id}:${b.state}`))});
