...
async function main() {
  ...
  try {
    while ((totalPages === null || page <= totalPages) && (!historical.historical || historicalInvocationHasBudget(providerRequests, historical.maxPages))) {
      ...
      const runMetadata = metadataFor(historical, { pagesFetched: providerRequests, earliest, latest, inRange, outOfRange, invalid, ordering: orderingState, complete: rangeComplete, resumePage: page + 1, effectiveStartPage: startPage, rateLimitLimit, rateLimitRemaining, rateLimitReset, rateLimitObservedAt });
    ...
    const finalMetadata = metadataFor(historical, { pagesFetched: providerRequests, earliest, latest, inRange, outOfRange, invalid, ordering: orderingState, complete: rangeComplete, resumePage: page, effectiveStartPage: startPage, rateLimitLimit, rateLimitRemaining, rateLimitReset, rateLimitObservedAt });
