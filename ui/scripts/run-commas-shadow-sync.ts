...
import { classifyHistoricalPageOverlap, combineOrdering, historicalChunkTransition, historicalInvocationHasBudget, historicalQuotaAllowed, historicalResumePage, inHistoricalRange, orderingForPage, parseHistoricalBackfillArgs, rangePassed, type OrderingState } from "../lib/commerce/commas-historical-backfill";
...
function metadataFor(args: ReturnType<typeof parseHistoricalBackfillArgs>, state: { pagesFetched: number; earliest: string | null; latest: string | null; inRange: number; outOfRange: number; invalid: number; ordering: OrderingState; complete: boolean; resumePage: number; effectiveStartPage?: number; rateLimitLimit?: string | null; rateLimitRemaining?: string | null; rateLimitReset?: string | null; rateLimitObservedAt?: string | null; boundaryOverlapCount?: number; paginationClassification?: string }) {
  return args.historical ? { historical_backfill: true, from_date: args.fromDate, to_date: args.toDate, start_page: state.effectiveStartPage ?? args.startPage, max_pages: args.maxPages, per_page: args.perPage, pages_fetched: state.pagesFetched, earliest_seen_timestamp: state.earliest, latest_seen_timestamp: state.latest, in_range_records: state.inRange, out_of_range_records: state.outOfRange, invalid_missing_date_records: state.invalid, ordering_state: state.ordering, range_complete: state.complete, resume_page: state.resumePage, rate_limit_limit: state.rateLimitLimit ?? null, rate_limit_remaining: state.rateLimitRemaining ?? null, rate_limit_reset: state.rateLimitReset ?? null, rate_limit_observed_at: state.rateLimitObservedAt ?? null, boundary_overlap_count: state.boundaryOverlapCount ?? 0, pagination_classification: state.paginationClassification ?? "none" } : {};
}
...
async function main() {
  ...
  let page = startPage, pagesCompleted = completed.size, recordsSeen = 0, recordsCreated = 0, recordsUpdated = 0, retries = 0, warnings = Number(run.warnings_count || 0), providerRequests = 0, totalPages: number | null = null, totalItems: number | null = null;
  ...
  let rateLimitLimit: string | null = historical.historical ? String(run.metadata?.rate_limit_limit || "") || null : null, rateLimitRemaining: string | null = historical.historical ? String(run.metadata?.rate_limit_remaining || "") || null : null, rateLimitReset: string | null = historical.historical ? String(run.metadata?.rate_limit_reset || "") || null : null, rateLimitObservedAt: string | null = historical.historical ? String(run.metadata?.rate_limit_observed_at || "") || null : null;
  const seenIds = new Set<string>();
  let previousPageLastId: string | null = null, boundaryOverlapCount = 0, paginationClassification = "none";
  try {
    while ((totalPages === null || page <= totalPages) && (!historical.historical || historicalInvocationHasBudget(providerRequests, historical.maxPages))) {
      ...
      const ids = parsed.items.map((item) => String(item.id)); const overlap = classifyHistoricalPageOverlap(previousPageLastId, ids, seenIds); if (overlap.classification === "pagination_instability" || overlap.repeatedCount > Math.max(2, Math.floor(ids.length * 0.1))) throw new Error("Major Commas pagination instability detected."); boundaryOverlapCount += overlap.boundaryOverlapCount; if (overlap.boundaryOverlapCount > 0) paginationClassification = "benign_boundary_overlap"; warnings += overlap.nonBoundaryRepeatCount; previousPageLastId = ids.at(-1) || previousPageLastId; ids.forEach((id) => seenIds.add(id));
      ...
      const runMetadata = metadataFor(historical, { pagesFetched: providerRequests, earliest, latest, inRange, outOfRange, invalid, ordering: orderingState, complete: rangeComplete, resumePage: page + 1, effectiveStartPage: startPage, rateLimitLimit, rateLimitRemaining, rateLimitReset, rateLimitObservedAt, boundaryOverlapCount, paginationClassification });
    ...
    const finalMetadata = metadataFor(historical, { pagesFetched: providerRequests, earliest, latest, inRange, outOfRange, invalid, ordering: orderingState, complete: rangeComplete, resumePage: page, effectiveStartPage: startPage, rateLimitLimit, rateLimitRemaining, rateLimitReset, rateLimitObservedAt, boundaryOverlapCount, paginationClassification });
