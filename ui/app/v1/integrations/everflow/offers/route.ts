...
import { listPersistedEverflowOffers } from "@/lib/integrations/everflow-offers";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!/^[0-9a-f-]{36}$/i.test(connectionId)) {
      return NextResponse.json({ ok: false, message: "A valid Everflow connection is required.", requestId }, { status: 400, headers: headers(requestId) });
    }
    ...
    const offers = await listPersistedEverflowOffers({ organizationId: resolution.session.activeOrganization.id, connectionId });
    return NextResponse.json({ ok: true, connectionId, offers, count: offers.length, requestId }, { headers: headers(requestId) });
  } catch {
    return NextResponse.json({ ok: false, message: "Everflow offers are unavailable.", requestId }, { status: 500, headers: headers(requestId) });
  }
}
