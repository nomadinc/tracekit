...
import { syncEverflowOffers } from "@/lib/integrations/everflow-offers";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, message: "TraceKit could not complete the Everflow offer sync.", requestId }, { status: 500, headers: responseHeaders(requestId) });
  }
}
