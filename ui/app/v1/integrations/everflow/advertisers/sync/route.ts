...
import { syncEverflowAdvertisers } from "@/lib/integrations/everflow-advertisers";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (resolution.kind !== "authenticated" || !resolution.session.activeOrganization) return NextResponse.json({ ok: false, message: "The requested resource is unavailable.", requestId }, { status: 404, headers: responseHeaders(requestId) });
    ...
    const result = await syncEverflowAdvertisers({ plane, session: resolution.session, organizationId: resolution.session.activeOrganization.id, connectionId });
    ...
    return NextResponse.json({ ok: false, message: "TraceKit could not complete the Everflow advertiser sync.", requestId }, { status: 500, headers: responseHeaders(requestId) });
  }
}
