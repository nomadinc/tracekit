...
import { listPersistedEverflowAffiliates } from "@/lib/integrations/everflow-affiliates";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const affiliates = await listPersistedEverflowAffiliates({
      organizationId: resolution.session.activeOrganization.id,
      connectionId,
    });
    return NextResponse.json({ ok: true, connectionId, affiliates, count: affiliates.length, requestId }, { headers: headers(requestId) });
  } catch {
    return NextResponse.json({ ok: false, message: "Everflow affiliates are unavailable.", requestId }, { status: 500, headers: headers(requestId) });
  }
}
