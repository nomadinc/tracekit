...
import { runStoredNext29ControlledOrderIncremental } from "@/lib/commerce/next29-production-certification";
...
export async function POST(request:Request){const requestId=randomUUID();try{
 ...
 const body=await request.json().catch(()=>null) as {connectionId?:unknown}|null; const connectionId=String(body?.connectionId??"").trim();
 if(!/^[0-9a-f-]{36}$/i.test(connectionId))return NextResponse.json({ok:false,code:"invalid_request",requestId},{status:400});
 const result=await runStoredNext29ControlledOrderIncremental({session:resolution.session,connectionId});
