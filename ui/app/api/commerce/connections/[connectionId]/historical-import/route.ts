...
export async function POST(request:Request,context:{params:Promise<{connectionId:string}>}){
 try{
  ...
  const result=await syncEverflowConversions({plane,session:resolution.session,organizationId:resolution.session.activeOrganization.id,connectionId,from:start+" 00:00:00",to:end+" 23:59:59"});
