...
export async function POST(request:Request,context:{params:Promise<{connectionId:string}>}){
 try{
  ...
  const existing=await commercePersistenceRequest(`commerce_sync_runs?connection_id=eq.${encodeURIComponent(connectionId)}&organization_id=eq.${encodeURIComponent(organizationId)}&sync_type=eq.everflow_conversions_historical&status=in.(queued,running)&select=id,status,lease_expires_at,metadata&order=created_at.desc&limit=1`) as Record<string,any>[];
  const active=existing[0];if(active?.status==="running"&&active.lease_expires_at&&Date.parse(String(active.lease_expires_at))<Date.now()){const resumable=Boolean(active.metadata?.from&&active.metadata?.to);if(!resumable){await commercePersistenceRequest(`commerce_sync_runs?id=eq.${encodeURIComponent(String(active.id))}&organization_id=eq.${encodeURIComponent(organizationId)}&connection_id=eq.${encodeURIComponent(connectionId)}`,{method:"PATCH",body:JSON.stringify({status:"failed",lease_owner:null,lease_expires_at:null,last_error_code:"everflow_historical_import_interrupted",last_error_summary:"Historical import worker stopped before completion and this legacy run cannot be resumed safely.",updated_at:new Date().toISOString()})});}else{return NextResponse.json({ok:true,code:"everflow_historical_import_recovering",message:"An interrupted historical Everflow import is awaiting recovery.",runId:active.id},{status:202});}}else if(active)return NextResponse.json({ok:true,code:"everflow_historical_import_already_active",message:"A historical Everflow import is already queued or running.",runId:active.id},{status:202});
