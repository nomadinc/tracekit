...
const confirmation = "resume-ordering-evidence-only";
...
const allowedCodes = new Set(["ordering_continuation_rpc_rejected", "ordering_continuation_rpc_result_invalid", "ordering_continuation_post_reservation_rejected", "ordering_continuation_queue_dispatch_failed", "ordering_continuation_dispatch_failed"]);
...
function safeCode(value: unknown) { const code = String(value || ""); return allowedCodes.has(code) ? code : "ordering_continuation_failed"; }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const upstream = await fetch(`${apiBase}/internal/commerce/resume-ordering-evidence-only`, { method: "POST", headers: { "content-type": "application/json", "x-tk-secret": secret }, body: JSON.stringify({ confirmation }) });
    ...
    return response(requestId, { ok: false, code: "ordering_continuation_failed" }, 500);
  }
}
