...
const QUERY = `query TraceKitShopifySmoke {
  ...
  products(first: 5, sortKey: UPDATED_AT, reverse: true) { nodes { id title updatedAt variants(first: 5) { nodes { id sku price } } } }
  customers(first: 5, sortKey: UPDATED_AT, reverse: true) { nodes { id createdAt updatedAt } }
  ...
  try {
    ...
    try {
      const response = await fetch(`https://${credential.shopDomain}/admin/api/${credential.apiVersion}/graphql.json`, {
        ...
        headers: { "content-type": "application/json", accept: "application/json", "x-shopify-access-token": credential.adminAccessToken, "x-correlation-id": resolution.session.correlationId },
        body: JSON.stringify({ query: QUERY }),
      });
      const payload = await response.json().catch(() => null) as any;
      if (!response.ok || payload?.errors?.length || !payload?.data?.shop?.id) {
        console.info("[shopify-smoke]", JSON.stringify({ connectionId, ok: false, status: response.status, graphqlErrorCount: Array.isArray(payload?.errors) ? payload.errors.length : 0 }));
        return NextResponse.json({ ok: false, error: "shopify_smoke_failed", status: response.status, graphqlErrors: Array.isArray(payload?.errors) ? payload.errors.map((item: any) => String(item?.message || "GraphQL error")).slice(0, 5) : [] }, { status: 502 });
      }
      ...
      const requestIdPresent = Boolean(response.headers.get("x-request-id") || response.headers.get("x-shopify-request-id"));
      const counts = { products: products.length, variants, customers: customers.length, orders: orders.length, lineItems, refunds };
      console.info("[shopify-smoke]", JSON.stringify({ connectionId, ok: true, shopDomain: credential.shopDomain, counts, requestIdPresent }));
      return NextResponse.json({ ok: true, shop: { id: payload.data.shop.id, name: payload.data.shop.name, domain: payload.data.shop.myshopifyDomain }, counts, samples: { products: products.map((product: any) => ({ id: product.id, title: product.title, updatedAt: product.updatedAt })), orders: orders.map((order: any) => ({ id: order.id, name: order.name, updatedAt: order.updatedAt, financialStatus: order.displayFinancialStatus, fulfillmentStatus: order.displayFulfillmentStatus })) }, requestIdPresent });
    } finally { clearTimeout(timeout); }
  } catch {
    console.info("[shopify-smoke]", JSON.stringify({ connectionId, ok: false, error: "server_error" }));
