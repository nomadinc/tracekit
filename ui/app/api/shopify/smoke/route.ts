...
const QUERY = `query TraceKitShopifySmoke {
  ...
  products(first: 5, sortKey: UPDATED_AT, reverse: true) {
    nodes { id title updatedAt variants(first: 5) { nodes { id sku price } } }
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      if (!response.ok || payload?.errors?.length || !payload?.data?.shop?.id) return NextResponse.json({ ok: false, error: "shopify_smoke_failed", status: response.status, graphqlErrors: Array.isArray(payload?.errors) ? payload.errors.map((item: any) => String(item?.message || "GraphQL error")).slice(0, 5) : [] }, { status: 502 });
      ...
      return NextResponse.json({ ok: true, shop: { id: payload.data.shop.id, name: payload.data.shop.name, domain: payload.data.shop.myshopifyDomain }, counts: { products: products.length, variants, customers: customers.length, orders: orders.length, lineItems, refunds }, samples: { products: products.map((product: any) => ({ id: product.id, title: product.title, updatedAt: product.updatedAt })), orders: orders.map((order: any) => ({ id: order.id, name: order.name, updatedAt: order.updatedAt, financialStatus: order.displayFinancialStatus, fulfillmentStatus: order.displayFulfillmentStatus })) }, requestIdPresent: Boolean(response.headers.get("x-request-id") || response.headers.get("x-shopify-request-id")) });
    } finally { clearTimeout(timeout); }
  } catch { return NextResponse.json({ ok: false, error: "shopify_smoke_failed" }, { status: 500 }); }
}
