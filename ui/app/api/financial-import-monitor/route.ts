...
export async function GET(req: Request) {
  ...
  if (!secret) {
    return NextResponse.json({
      ...
      message: "TK_SECRET_KEY is required on the UI server for Financial Import Monitor requests.",
    }, { status: 500 });
  }

  const url = new URL(req.url);
  const search = url.searchParams.toString();
  const res = await fetch(`${apiBaseUrl()}/v1/financial-import-monitor${search ? `?${search}` : ""}`, {
    method: "GET",
    cache: "no-store",
    headers: {
