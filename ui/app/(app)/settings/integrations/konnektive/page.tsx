// ui/app/(app)/settings/integrations/konnektive/page.tsx
"use client";
...
const DEFAULT_BASE_URL = "https://api.checkoutchamp.com";
...
export default function CheckoutChampIntegrationPage() {
  // Connection fields
  ...
  React.useEffect(() => {
    ...
    setViewOrdersHref(buildOrdersUrl("checkoutchamp", li.from, li.to));
    setImpStatus("done");
    setImpMsg(
  ...
  async function refreshStatus() {
    ...
    setStatusErr(null);
    try {
      const json = await apiGetJson<StatusResponse>("/v1/integrations/checkoutchamp/status");
      setStatusResp(json);
      if (!json.ok) {
  ...
  async function loadSettings() {
    ...
    setSettingsMsg(null);
    try {
      const json = await apiGetJson<SettingsGetResponse>("/v1/integrations/checkoutchamp/settings");
  ...
  async function saveSettings() {
    ...
    try {
      const json = await apiPostJson<SettingsSaveResponse>(
        "/v1/integrations/checkoutchamp/settings",
        {
  ...
  try {
    ...
    const json = await apiPostJson<RunNowResponse>("/v1/integrations/checkoutchamp/run-now", {
      from,
      to,
  ...
  async function handleImportOrders() {
    ...
    try {
      ...
      const href = buildOrdersUrl("checkoutchamp", from, to, impFilter === "all_sales" ? undefined : impFilter.toUpperCase());
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-lg font-semibold">CheckoutChamp</h1>
      ...
      <div className="rounded-xl border p-4 space-y-3 max-w-2xl">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-slate-500">
              Pulls transactions from CheckoutChamp and upserts into{" "}
