// ui/app/(app)/settings/integrations/wowpay/page.tsx
"use client";
...
const DEFAULT_BASE_URL = "https://api.wowpay.com";
...
const LAST_IMPORT_KEY = "tracekit:lastImport:wowpay";
...
function safeLoadLastImport(): LastImportState | null {
  try {
    ...
    if (!v || v.platform !== "wowpay" || !v.from || !v.to || !v.importedAt) return null;
    return {
      platform: "wowpay",
      from: String(v.from),
      to: String(v.to),
...
export default function WowPayIntegrationPage() {
  ...
  React.useEffect(() => {
    ...
    setViewOrdersHref(buildOrdersUrl("wowpay", li.from, li.to));
    setImpStatus("done");
    setImpMsg(
  ...
  async function refreshStatus() {
    ...
    setStatusErr(null);
    try {
      const json = await apiGetJson<StatusResponse>("/v1/integrations/wowpay/status");
      setStatusResp(json);
      if (!json.ok) {
  ...
  async function loadSettings() {
    ...
    setSettingsMsg(null);
    try {
      const json = await apiGetJson<SettingsGetResponse>("/v1/integrations/wowpay/settings");
  ...
  try {
    ...
    const json = await apiPostJson<RunNowResponse>("/v1/integrations/wowpay/run-now", {
      from,
      to,
  ...
  async function handleImportOrders() {
    ...
    try {
      ...
      const href = buildOrdersUrl("wowpay", from, to, impFilter === "all_sales" ? undefined : impFilter.toUpperCase());
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-lg font-semibold">wowpay</h1>
      ...
      <div className="rounded-xl border p-4 space-y-3 max-w-2xl">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-slate-500">
              Pulls transactions from wowpay and upserts into{" "}
