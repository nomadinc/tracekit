...
test("surfaces Shopify bulk user errors", async () => {
  const fetchImpl: typeof fetch = async () => new Response(JSON.stringify({
    data: {
      bulkOperationRunQuery: {
        bulkOperation: null,
        userErrors: [{ field: ["query"], message: "A bulk query operation for this app and shop is already in progress." }],
      },
    },
  }), { status: 200, headers: { "content-type": "application/json" } });
  const reader = createShopifyBulkReader({ shopDomain: "example.myshopify.com", accessToken: "test-token", fetchImpl });
  await assert.rejects(
    () => reader.start({ resource: "customers", before: "2026-01-01T00:00:00Z" }),
    /already in progress/,
  );
});

test("reads current bulk operation state", async () => {
  const fetchImpl: typeof fetch = async () => new Response(JSON.stringify({
    data: {
  ...
  const reader = createShopifyBulkReader({ shopDomain: "example.myshopify.com", accessToken: "test-token", fetchImpl });
  const operation = await reader.current();
...
test("parses flat bulk JSONL resources", () => {
  const result = parseShopifyBulkJsonl("customers", [
    JSON.stringify({ id: "gid://shopify/Customer/1", email: "a@example.com", updatedAt: "2026-01-01T00:00:00Z" }),
    JSON.stringify({ id: "gid://shopify/Customer/2", email: "b@example.com", updatedAt: "2026-01-02T00:00:00Z" }),
  ].join("\n"));
...
test("reassembles order children from Shopify JSONL parent relationships", () => {
  ...
  const result = parseShopifyBulkJsonl("orders", [
    JSON.stringify({ id: orderId, name: "#1001", updatedAt: "2026-01-01T00:00:00Z" }),
    JSON.stringify({ id: "gid://shopify/LineItem/1", __parentId: orderId, __typename: "LineItem", quantity: 2 }),
    JSON.stringify({ id: "gid://shopify/OrderTransaction/1", __parentId: orderId, __typename: "OrderTransaction", status: "SUCCESS" }),
    JSON.stringify({ id: "gid://shopify/Refund/1", __parentId: orderId, __typename: "Refund", createdAt: "2026-01-02T00:00:00Z" }),
  ].join("\n"));
