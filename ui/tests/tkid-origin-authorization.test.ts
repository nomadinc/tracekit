...
test("authorization fails closed when organization, context, or capabilities are missing",()=>{
  ...
  const noFeatureAccess=session({resourceType:"tkid_origin_registry"}); noFeatureAccess.effectivePermissions=noFeatureAccess.effectivePermissions.filter((permission)=>permission!=="admin.manage_feature_access");
  assert.equal(canManageTkidOrigins(noFeatureAccess),false);
  assert.throws(()=>requireTkidOriginManagement(noFeatureAccess),/unavailable/);
});

test("an effective deny remains fail-closed despite a matching allow override",()=>{
  const denied=session({role:"organization-owner",resourceType:"tkid_origin_registry"});
  denied.permissionOverrides.push({...denied.permissionOverrides[0],id:"override-deny",effect:"deny"});
  denied.effectivePermissions=denied.effectivePermissions.filter((permission)=>permission!=="admin.manage_feature_access");
  assert.equal(canManageTkidOrigins(denied),false);
  assert.throws(()=>requireTkidOriginManagement(denied),/unavailable/);
});
