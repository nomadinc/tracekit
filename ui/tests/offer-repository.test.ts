...
import { normalizeOfferDeepLink, offerDeepLinkHref, parseOfferDeepLink } from "../lib/offers/deep-link";
...
const scope = (id = "client-admin", organizationId: string | null = "org-bullseye") => ({ authenticated: true, identity: identity(id), organizationId });
...
test("mock Offer snapshots are scoped, serializable, and freshly cloned", async () => {
  ...
  assert.deepEqual(offers.map(item => item.organizationId), ["org-bullseye", "org-bullseye"]);
...
test("Offer scope rejects inaccessible Offers and product-admin client rendering", async () => {
  const repository = new MockOfferRepository();
  assert.equal(await repository.loadWorkspace(scope(), "offer-valuerx-individual"), null);
  assert.deepEqual(await repository.listOffers(scope("platform-admin", null)), []);
  assert.equal(await repository.resolveOffer(scope("client-admin", null), "offer-valuerx-individual"), null);
  assert.deepEqual(await repository.resolveOffer(scope("agency-owner", null), "offer-valuerx-individual"), { organizationId: "org-valuerx", offerId: "offer-valuerx-individual" });
});
...
test("deep links parse and normalize invalid route state safely", async () => {
  ...
  const parsed = parseOfferDeepLink("?offer_id=missing&focus=profit-drivers&driver=missing&event_id=missing&drawer=driver:missing&compare=1&compare_offer=missing");
...
test("Mission Control Offer hrefs preserve identity and production focus", () => {
  const href = offerDeepLinkHref({ offerId: "offer-bullseye", focus: "profit-drivers", driverId: "actual-shipping-cost", drawerId: "driver:actual-shipping-cost" }, "client-admin");
...
test("Compare accepts two to four valid accessible Offers and derives its conclusion", async () => {
  ...
  const comparison = await repository.loadComparison(agencyScope, ["offer-valuerx-individual", "offer-valuerx-family", "offer-valuerx-individual", "offer-bullseye"]);
  ...
  assert.equal(await repository.loadComparison(agencyScope, ["offer-valuerx-individual"]), null);
});

test("trend selection and Offer search resolve through repository use cases", async () => {
  const repository = new MockOfferRepository();
  const trend = await repository.loadTrend(scope(), "offer-bullseye", 14, ["profit", "cpa"]);
  assert.equal(trend.length, 14);
  assert.ok(trend.every(point => typeof point.profit === "number" && typeof point.cpa === "number"));
