...
| `integrations_credentials.password_key_version` lineage constraint | 059 | ADDITIVE / SERVER-ONLY | Deterministic meanings: 1=Legacy B, 2=future rotation key, 3=decrypt-only Legacy C. Migration 059 embeds no row identifiers or encrypted fields; the two-row C classification is a separate fail-closed operator transaction. |
